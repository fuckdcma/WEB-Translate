...
const cooldownMinutes=Math.max(5,Number(process.env.RESUME_COOLDOWN_MINUTES)||8);
const nextSchedule=(notBefore=Date.now())=>{const date=new Date(Math.max(Date.now(),Number(notBefore)||0));date.setUTCSeconds(0,0);date.setUTCMinutes(Math.ceil(date.getUTCMinutes()/10)*10);return date.toISOString()};
...
async function main(){
  ...
  for(const project of projects||[]){
    ...
    if(Date.now()-when(dispatch.queuedAt)<cooldownMinutes*60_000||Date.now()-when(automation?.lastAttemptAt)<cooldownMinutes*60_000)continue;
