...
const nextSchedule=()=>{const date=new Date();const minute=date.getUTCMinutes();date.setUTCSeconds(0,0);date.setUTCMinutes(minute<7?7:minute<37?37:67);return date.toISOString()};
...
async function main(){
  ...
  for(const project of projects||[]){
    ...
    const workerCount=Math.min(4,Math.max(1,Math.ceil(Number(project.totalRows||0)/500)));
    ...
    await uploadWithRetry([jsonFile(`status/${project.id}/dispatch.json`,{status:'queued',runId,workers:workerCount,requestedWorkers:workerCount,architecture:'task-queue-v2',automatic:true,queuedAt:now}),jsonFile(`status/${project.id}/automation.json`,{enabled:true,status:'dispatched',runId,lastAttemptAt:now,nextAttemptAt:nextSchedule(),message:'Đã tự động tiếp tục từ checkpoint đã lưu.'})]);
