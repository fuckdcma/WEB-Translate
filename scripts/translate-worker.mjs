...
async function main(){
  try{
    ...
    await uploadWithRetry([...migrated,workerStatus('running',{model:primaryModel,...progress(),queuedBatches:queue.length,message:`Đang dịch ${dataRows.length-skippedIndexes.size} dòng có nội dung; giữ nguyên ${skippedIndexes.size} dòng trống`})]);
    while(queue.length){
...
${compact}`;
      try{
        ...
        files.push(...router.files(),workerStatus('running',{model:response.model,...progress(),currentCategory:category,inputTokens:response.inputTokens,queuedBatches:queue.length,message:`Đã xử lý ${translations.size+skippedIndexes.size}/${dataRows.length} dòng · ${skippedIndexes.size} dòng trống giữ nguyên`}));await uploadWithRetry(files);
