...
async function main(){
  try{
    ...
    const queue=[];for(const category of ['menu','interaction','story'])queue.push(...compactBatches(pending.filter(item=>item.category===category),{tokenBudget:router.batchTokenBudget(),maxRows:router.batchRowLimit(),serialize:item=>`${item.id}\t${item.text}`}));
...
${compact}`;
    ...
    const completedAt=new Date().toISOString();await uploadWithRetry([...router.files(),workerStatus('completed',{model:primaryModel,...progress(),completedAt,message:'Đã dịch xong queue và lưu toàn bộ checkpoint'})]);await setReady(true);console.log(`Translated and saved ${translations.size}/${dataRows.length} rows`);
