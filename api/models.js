...
async function usageToday(paths){
  ...
  const ledgerModels=new Set();const ledgerPaths=paths.filter(path=>path.startsWith(`usage/google/${day}/`)&&/\.json$/i.test(path));for(const ledger of await readMany(ledgerPaths)){if(!ledger?.model)continue;ledgerModels.add(ledger.model);const item=blank(ledger.model);item.requests+=Number(ledger.requests)||0;item.inputTokens+=Number(ledger.inputTokens)||0;item.outputTokens+=Number(ledger.outputTokens)||0;item.totalTokens+=Number(ledger.totalTokens)||0;for(const event of ledger.events||[])if(Date.now()-Date.parse(event.at)<60_000){item.minuteRequests+=1;item.minuteInputTokens+=Number(event.inputTokens)||0}}
  const legacyPaths=paths.filter(path=>/^checkpoints\/[^/]+\/(?:(?:tasks|partial)\/[^/]+|review)\.json$/i.test(path));let estimatedRequests=0;for(const record of await readMany(legacyPaths)){const timestamp=record?.completedAt||record?.updatedAt;if(record?.version>=3||!timestamp||quotaDay(timestamp)!==day)continue;const model=record.model||'legacy';if(ledgerModels.has(model))continue;const item=blank(model);const usage=record.usage||{};item.inputTokens+=Number(usage.inputTokens)||0;item.outputTokens+=Number(usage.outputTokens)||0;item.totalTokens+=Number(usage.totalTokens)||0;if(Number(usage.requests)>0)item.requests+=Number(usage.requests);else if(record.complete){item.requests+=1;estimatedRequests+=1}}
