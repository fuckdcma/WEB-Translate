...
async function main(){
  try{
    ...
    const sourceHash=createHash('sha256').update(source).digest('hex');const delimiter=/\.csv$/i.test(project.fileName)?',':'\t';const rows=parseDelimited(source,delimiter);const columns=detectColumns(rows);const dataRows=rows.slice(1);const {taskSize,tasks,categories}=buildTaskPlan(dataRows.map(row=>({key:row[columns.key],text:row[columns.source]})));const sendableRows=dataRows.map((row,index)=>({id:index+1,text:String(row[columns.source]||'')})).filter(item=>item.text.trim());const compactSource=sendableRows.map(item=>`${item.id}\t${item.text}`).join('\n');const estimatedInputTokens=Math.max(1,Math.ceil(compactSource.length/4));const skippedRows=dataRows.length-sendableRows.length;const createdAt=new Date().toISOString();
    const manifest={version:3,projectId,runId,sourceHash,delimiter,taskSize,workerCount,model,googleConfig,totalRows:dataRows.length,translatableRows:sendableRows.length,skippedRows,totalTasks:tasks.length,estimatedInputTokens,categories,tasks,createdAt};
    await uploadWithRetry([jsonFile(`queues/${projectId}/${runId}/manifest.json`,manifest),jsonFile(`status/${projectId}/coordinator.json`,{runId,status:'completed',architecture:'quota-queue-v3',taskSize,workerCount,model,totalTasks:tasks.length,totalRows:dataRows.length,translatableRows:sendableRows.length,skippedRows,estimatedInputTokens,categories,completedAt:createdAt})]);
