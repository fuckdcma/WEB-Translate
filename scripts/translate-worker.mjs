...
async function main(){
  try{
    ...
    const plan=tokenBatchPlan(pending,{tokenBudget:router.batchTokenBudget(),serialize:item=>`${item.id}\t${item.category}\t${item.text}`});const queue=[...plan.batches];
    ...
    await uploadWithRetry([...migrated,workerStatus('running',{model:primaryModel,...progress(),queuedBatches:queue.length,plannedBatches:plan.batchCount,pendingTokens:plan.totalTokens,tokenBudget:plan.tokenBudget,message:`Đã chia ${plan.totalTokens.toLocaleString('vi-VN')} token thành ${plan.batchCount} lô theo 90% TPM`})]);
    while(queue.length){
...
${compact}`;
      try{
        ...
        const missing=batch.filter(item=>!translations.has(item.index));if(missing.length)queue.unshift(...tokenBatchPlan(missing,{tokenBudget:router.batchTokenBudget(),serialize:item=>`${item.id}\t${item.category}\t${item.text}`}).batches);
      }catch(error){
