...
function formatAge(seconds){const value=Math.max(0,Math.round(Number(seconds)||0));if(value<60)return`${value} giây`;if(value<3600)return`${Math.round(value/60)} phút`;return`${Math.round(value/360)/10} giờ`}
...
async function loadRuns(){const list=$('#runsList');try{const data=await request('/api/agent-runs');runs=data.runs||[];const running=runs.filter(x=>x.status==='in_progress').length,queued=runs.filter(x=>x.status==='queued').length;$('#runningCount').textContent=running;$('#queuedCount').textContent=queued;$('#actionCount').textContent=running+queued;list.innerHTML=runs.length?runs.slice(0,4).map(run=>{const success=run.status==='completed';const failed=['failed','cancelled'].includes(run.status);return `<div class="timeline-item"><span class="timeline-icon ${success?'success':failed?'failed':'process'}">${success?'✓':failed?'!':'↻'}</span><div><strong>${escapeHtml(run.name)}</strong><p>${escapeHtml(run.detail||run.stage||run.status)}${run.currentFile||run.sourceFile?` · ${escapeHtml(run.currentFile||run.sourceFile)}`:''}</p><small>${new Date(run.updatedAt||run.createdAt).toLocaleString('vi-VN')}${run.nextRunAt&&run.status==='queued'?` · tự tiếp tục ${new Date(run.nextRunAt).toLocaleTimeString('vi-VN',{hour:'2-digit',minute:'2-digit'})}`:''}</small></div></div>`}).join(''):'<div class="empty-state compact-empty"><span>◇</span><p>Chưa có phiên xử lý nền.</p></div>'}catch(error){$('#actionCount').textContent='—';list.innerHTML=`<div class="empty-state compact-empty"><span>!</span><p>${escapeHtml(error.message)}</p></div>`}}
async function loadIntegrations(){try{const data=await request('/api/integrations');setIntegration('#hfStatus',data.checks.huggingFace);setIntegration('#agentStatus',data.checks.agentHooks);setIntegration('#googleStatus',data.checks.googleAI);$('#hfRepo').textContent=data.dataset||'Chưa cấu hình';$('#agentProvider').textContent=data.checks.agentHooks?.runner||'antigravity-preview-05-2026';$('#agentModel').textContent=data.agentModel||'gemini-3.5-flash-lite';$('#googleModel').textContent=data.model||'gemini-3.6-flash'}catch(error){showToast(error.message,true)}}
...
function renderQuotaMonitor(data){
  const status=$('#monitorStatus');status.className=`monitor-status ${data.status||'waiting'}`;status.textContent=data.status==='connected'?'✓ Đã đối chứng':data.status==='stale'?'Chưa có bản đồng bộ mới':data.status==='setup_required'?'Chờ cấp quyền':data.status==='error'?'Chưa đọc được':'Đang chờ dữ liệu';$('#monitorMessage').textContent=data.message||data.error||'Đang lấy dữ liệu đối chứng';$('#monitorProject').textContent=data.projectId||'—';$('#monitorSynced').textContent=data.syncedAt?new Date(data.syncedAt).toLocaleTimeString('vi-VN'):'—';$('#monitorObserved').textContent=data.newestAt?new Date(data.newestAt).toLocaleTimeString('vi-VN'):'—';$('#monitorLag').textContent=Number.isFinite(Number(data.lagSeconds))?formatAge(Number(data.lagSeconds)):'—';
  const googleDaily=Number(data.google?.rpd?.usage);const googleInput=Number(data.google?.tpd?.usage);const internalDaily=Number(data.internal?.rpd)||0;if(Number.isFinite(googleDaily)){$('#dailyRequests').textContent=googleDaily.toLocaleString('vi-VN');$('#dailyRequestSource').textContent=`Google · hệ thống giữ ${internalDaily.toLocaleString('vi-VN')}`}if(Number.isFinite(googleInput)){$('#dailyTokens').textContent=googleInput.toLocaleString('vi-VN');$('#dailyTokenSource').textContent='Google ghi nhận theo ngày quota (giờ Thái Bình Dương)'}
  for(const dimension of ['rpm','tpm','rpd']){const key=dimension[0].toUpperCase()+dimension.slice(1);const item=data.comparison?.[dimension]||{};const local=Number(item.internal)||0;const google=item.google===null||item.google===undefined?null:Number(item.google);const limit=item.limit===null||item.limit===undefined?null:Number(item.limit);$(`#local${key}`).textContent=quotaValue(local);$(`#google${key}`).textContent=google===null?'—':quotaValue(google);$(`#${dimension}CompareBar`).style.width=`${Math.min(100,Math.max(0,Number(item.googlePercent)||0))}%`;const difference=$(`#${dimension}Difference`);if(google===null){difference.textContent='Đang chờ số Google';difference.className=''}else if(dimension==='rpd'){difference.textContent=`Google ${quotaValue(google)} · hệ thống giữ ${quotaValue(local)}${limit?` · giới hạn ${quotaValue(limit)}`:''}`;difference.className=Math.abs(local-google)>Math.max(2,google*.25)?'warn':'match'}else{difference.textContent=`Cao nhất hôm nay ${quotaValue(google)}${limit?` · giới hạn ${quotaValue(limit)}`:''}`;difference.className='match'}}
}
...
function renderTracker(data){
  ...
  $('#workerSummary').textContent=data.coordinator&&data.totalTasks?`${Number(data.completedTasks||0)}/${data.totalTasks} checkpoint · khoảng ${Number(data.coordinator.estimatedInputTokens||0).toLocaleString('vi-VN')} token nguồn · ${data.summary.completed}/${data.workers} luồng xong${data.summary.paused?` · ${data.summary.paused} tạm dừng`:''}`:data.workers?`${data.summary.completed}/${data.workers} hoàn tất${data.summary.paused?` · ${data.summary.paused} tạm dừng`:''}`:'Đang chờ';
  $('#workerGrid').innerHTML=data.shards.length?data.shards.map(item=>`<div class="worker-chip ${item.status}" title="${escapeHtml(item.error||item.message||'')}"><i>${item.status==='completed'?'✓':item.status==='failed'?'!':item.status==='paused'?'Ⅱ':item.status==='running'?'↻':'·'}</i><span>Luồng ${Number(item.index)+1}<small>${item.totalTasks?`${Number(item.completedTasks||0)}/${item.totalTasks} checkpoint${Number(item.queuedBatches)>0?` · ${Number(item.queuedBatches)} lô còn lại`:''}`:item.status==='completed'?'Hoàn tất':item.status==='failed'?'Lỗi':item.status==='paused'?`Đã lưu ${Number(item.processedRows||0)} dòng`:item.status==='running'?`${Number(item.processedRows||0)} dòng`:'Chờ'}</small></span></div>`).join(''):'<span class="worker-empty">Chưa có luồng xử lý</span>';
...
bindDynamicActions();loadProjects();loadRuns();runsTimer=setInterval(()=>{if($('#overview').classList.contains('active'))loadRuns()},15_000);
