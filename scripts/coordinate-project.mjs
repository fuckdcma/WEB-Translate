...
async function main(){
  try{
    ...
    const manifest={version:3,projectId,runId,sourceHash,delimiter,taskSize,workerCount,model,googleConfig,totalRows:dataRows.length,totalTasks:tasks.length,categories,tasks,createdAt};
    await uploadWithRetry([jsonFile(`queues/${projectId}/${runId}/manifest.json`,manifest),jsonFile(`status/${projectId}/coordinator.json`,{runId,status:'completed',architecture:'quota-queue-v3',taskSize,workerCount,model,totalTasks:tasks.length,totalRows:dataRows.length,categories,completedAt:createdAt})]);
