...
export default async function handler(req,res){
  ...
  try{
    ...
    if(!googleCloudMonitorConfigured())return send(res,200,{configured:false,status:'setup_required',model,internal,google:null,comparison:comparison(internal,null),message:'Cần cấp quyền chỉ đọc Google Cloud để lấy số đối chứng thực tế.'});
    const force=String(req.query?.refresh||'')==='1';if(force||!cached||Date.now()-cached.at>60_000){cached={at:Date.now(),value:await fetchGoogleQuotaSnapshot()}}
    const snapshot=cached.value;const google=findGoogleModel(snapshot,model);const lag=Number(snapshot.lagSeconds);const status=!google?'waiting':Number.isFinite(lag)&&lag>600?'stale':'connected';send(res,200,{configured:true,status,model,internal,google,comparison:comparison(internal,google),projectId:snapshot.projectId,syncedAt:snapshot.syncedAt,newestAt:snapshot.newestAt,lagSeconds:snapshot.lagSeconds,sources:snapshot.sources,errors:snapshot.errors,message:google?'Đang đối chứng số realtime nội bộ với số Google Cloud ghi nhận.':'Google Cloud chưa trả dữ liệu cho model này.'});
