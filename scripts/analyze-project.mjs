...
async function main(){
  try{
    ...
    let pending=items.filter(item=>!entries.has(item.id));let queue=compactBatches(pending,{tokenBudget:router.batchTokenBudget(),maxRows:1_000,serialize:item=>`${item.id}\t${item.text}`});
...
${compact}`;
      try{
        ...
        const missing=batch.filter(item=>!entries.has(item.id));if(missing.length)queue.unshift(...compactBatches(missing,{tokenBudget:Math.max(500,Math.floor(router.batchTokenBudget()/2)),maxRows:Math.max(10,Math.floor(batch.length/2)),serialize:item=>`${item.id}\t${item.text}`}));
      }catch(error){
