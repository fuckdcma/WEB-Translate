...
${JSON.stringify(batch)}`;
      let response;
      try{response=await requestGoogle({prompt,temperature:0.1,label:'Google AI final review',model:googleModel});apiRequests+=response.attempts;addUsage(usage,response.usage,1)}
      catch(error){if(!(error instanceof GooglePauseError))throw error;apiRequests+=Number(error.attempts)||1;usage.requests+=Number(error.attempts)||1;const pausedAt=new Date().toISOString();const checkpoint={version:2,projectId,runId,model:googleModel,sourceHash:manifest.sourceHash,checkedRows,issues,corrections:[...corrections].map(([index,translation])=>({index,translation})),apiRequests,usage,updatedAt:pausedAt};const progress=dataRows.length?Math.round(checkedRows/dataRows.length*100):100;const files=[jsonFile(checkpointPath,checkpoint),statusFile('paused',{model:googleModel,progress,checkedRows,totalRows:dataRows.length,issueCount:issues.length,correctionCount:corrections.size,apiRequests,usage,pauseReason:error.reason,message:error.message,pausedAt})];if(error.reason==='daily_quota')files.push(jsonFile(`status/${projectId}/quota.json`,{runId,model:googleModel,reason:'daily_quota',quota:error.quota,message:error.message,pausedAt,stage:'review'}));await uploadWithRetry(files);console.warn(error.message);return}
    ...
    dataRows.forEach((row,index)=>{row[columns.target]=corrections.get(index)||translations.get(index)||''});const output=serializeDelimited(rows,manifest.delimiter);const completedAt=new Date().toISOString();const report={version:2,projectId,runId,model:googleModel,sourceHash:manifest.sourceHash,checkedRows:dataRows.length,issueCount:issues.length,correctionCount:corrections.size,issues,corrections:[...corrections].map(([index,translation])=>({index,translation})),categories:manifest.categories,apiRequests,usage,completedAt};const checkpoint={...report,complete:true,updatedAt:completedAt};
