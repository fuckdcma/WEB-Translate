...
function normalizeLimits(profile={}){const configured=Number(profile.rpm)>0&&Number(profile.tpm)>0&&Number(profile.rpd)>0;return{rpm:Math.floor(Number(profile.rpm)>0?Number(profile.rpm):1),tpm:Math.floor(Number(profile.tpm)>0?Number(profile.tpm):8_000),rpd:Math.floor(Number(profile.rpd)>0?Number(profile.rpd):10),reservePercent:clamp(profile.reservePercent||20,5,50),source:profile.source||(configured?'configured':'safe_default'),configured}}
